...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            badgeLayer.frame = CGRectMake(imageWidth - 10, 0, 10, 10);
            badgeLayer.backgroundColor = UIColor.redColor.CGColor;
            badgeLayer.cornerRadius = 5.0;
            badgeLayer.hidden = YES;
            [imageLayer addSublayer:badgeLayer];
            [badges addObject:badgeLayer];
            for (NSNumber *number in self.badgeIdxs) {
                if([number integerValue] == idx) {
...
- (void)showBadgeWithIndex:(NSInteger)idx {
    if (self.badgeArray) {
        CALayer *layer = [self.badgeArray objectAtIndex:idx];
        layer.hidden = NO;
    }
}

- (void)hideBadgeWithIndex:(NSInteger)idx {
    NSMutableArray *tempArr = [NSMutableArray arrayWithArray:self.badgeIdxs];
    for (NSNumber *number in self.badgeIdxs) {
        if ([number integerValue] == idx) {
            [tempArr removeObject:number];
        }
    }
    
    self.badgeIdxs = tempArr;
    [self setNeedsDisplay];
}
...
- (void)setBadgeIdxs:(NSArray *)badgeIdxs {
    _badgeIdxs = badgeIdxs;
    [self setNeedsDisplay];
}
